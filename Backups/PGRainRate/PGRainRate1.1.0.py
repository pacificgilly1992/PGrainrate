...
PGfilelog = np.zeros_like(pgfile)
PGfilelog2 = np.zeros_like(pgfile)

for i in xrange(len(pgfile)):
	for j in xrange(len(pgfilelog)):
		if os.path.basename(pgfile[i])[:-4]==os.path.basename(pgfilelog[j])[:-4]:
			PGfilelog[i] = pgfilelog[j]
...
try:
	for i in range(len(pgfile)):
		it+=1
		try:
			slope1, intercept1, r_value1, p_value1, std_err1, pearson_cor1, RainRateBin1, TimeTipBin1, PGTipBin1, day1 = PGrainrate(pgfile[i], bincount, PGfilelog2[i])
